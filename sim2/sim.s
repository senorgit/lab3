...
;------Assembler Directives----------------
		...
		EXPORT	FLAG [DATA,SIZE=4]	; export public varialbe "ABS" for use elsewhere 
...
FLAG	SPACE	4					; allocates 4 uninitialized bytes in RAM for LARGER

		; Code
		AREA    |.text|, CODE, READONLY, ALIGN=2	; code in flash ROM
		EXPORT  Start				; export public function "start" for use elsewhere
NUM1   	DCD   	4294967295					; 32-bit constant data NUM1 = -1
...
GET_LARGER							; subroutine GET_LARGER		
		...
		STR R1, [R3]				; assume NUM1 is the larger number
		SUBS R0, R1, R2				; R0=R1-R2
		BLT GET_SWITCH				; if R0 <= 0, branch to GET_SWITCH and set NUM2 as the larger number
		BEQ GET_FLAG
		BX	LR						; subroutine return
GET_SWITCH
		STR R2, [R3]				; set NUM2 as the larger number
		BX LR
GET_FLAG
		LDR R3, =FLAG
		MOV R1, #1
